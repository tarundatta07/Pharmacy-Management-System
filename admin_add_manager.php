		...
		if(isset($_POST['submit']))
		{
			...
			if($password!=$cpass)
			{
				echo '<script type="text/javascript">
							...
							window.location.href = "admin_add_manager.php";
						   </script>';
			}
			else
			{	
				// Checking if username already exists 
				$sql1=mysqli_query($con,"SELECT * FROM manager WHERE username='$username'")or die(mysqli_error());
	 			...
	 			{
					$sql="INSERT INTO manager (manager_id, username, password, first_name, last_name, address, phone, email_id, date) VALUES ('$manager_id', '$username', '$password', '$fname', '$lname', '$address', '$mobile', '$email', NOW())";
					...
					if(mysqli_query($con,$sql)) 
					{
						echo '<script type="text/javascript">
								var f;        
						 		...
						 		var l;
						 		l = "'.$lname.'";
								alert("New Manager : \""+f+" "+l+"\" has been added Successfully!");
					...
					else
					{
						...
						echo '<script type="text/javascript">
							      var e;        
							      e = "'.$error.'";
							      alert("Inserting Manager Failed, Try Again! \nERROR : "+e);
							      window.location.href = "admin_add_manager.php";
							 </script>';
					}
				}
...
<html>
<head>
	<title> ADMIN ADD MANAGER PAGE </title>
	...
	<script type="text/javascript">
		...
		function validate(textbox)
		{
			...
			if(a == "Password")
			{
				if(n == "")
				{
					textbox.setCustomValidity('Please Enter Manager\'s Password');
				}
				else if(n.length<8)
				{
			...
			if(a == "Confirm Password")
			{
				if(n == "")
				{
					textbox.setCustomValidity('Please Re-enter Manager\'s Password');
				}
				else if(n.length<8)
				{
					textbox.setCustomValidity('Password Length must be atleast 8 Characters');
	...
	<div id="homepage">
		...
		<div id="mainarea">
				...
				<div id="mainbox" align="center">
					<h4> ADMIN - ADD NEW MANAGER </h4>
				<form name="add_manager" id="add_manager" action="admin_add_manager.php" method="post" autocomplete="OFF">
					<table>
        				<tr> 
        					<th>Manager ID</th> 
        					<td> <input type="text" name="id" placeholder="Manager ID" oninvalid="validate(this);" oninput="validate(this);" required=""> </td>
        				</tr>
        				<tr>
        					<th>First name </th> 
        				...
        				<tr>   
        					<th>Address </th> 
        					<td><textarea name="address" rows="3" form="add_manager" placeholder="Address" wrap="soft" oninvalid="validate(this);" oninput="validate(this);" required=""></textarea> </td>
        				</tr>
        				<tr>   
        					<th>Mobile No.</th> 
        			...
        			<input type="submit" class="button" name="submit" value="ADD MANAGER"> <br> &nbsp;
				</form>
				</div>
		</div>
