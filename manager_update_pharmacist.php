...
<?php
		...
		// GET PHARMACIST ID FROM URl
		if(isset($_GET['pharmacist_id']))
		{
			$pid=$_GET['pharmacist_id'];

			// GET DETAILS OF PHARMACIST TO BE UPDATED
			$result = mysqli_query($con,"SELECT username, first_name, last_name, address, phone, email_id FROM pharmacist where pharmacist_id='$pid'")or die(mysqli_error());
		...
		if(isset($_POST['submit']))
		{
			...
			$sql2 = mysqli_query($con,"SELECT username FROM pharmacist where pharmacist_id='$s_pid123'")or die(mysqli_error());
			...
			if($s_username!=$p)
			{
				...
				$sql1=mysqli_query($con,"SELECT username FROM pharmacist WHERE username='$s_username'")or die(mysqli_error());
	 			...
	 			if($result1>0)
	 			{
					echo '<script type="text/javascript">
							...
							window.location.href = "manager_update_pharmacist.php?pharmacist_id="+u1;
						   </script>';
	 			}
 			}

				$sql="UPDATE pharmacist SET pharmacist_id='$s_pharmacist_id', username='$s_username', first_name='$s_fname', last_name='$s_lname', address='$s_address', phone='$s_mobile', email_id='$s_email' WHERE pharmacist_id='$s_pid123'";
				if(mysqli_query($con,$sql))
				{
					echo '<script type="text/javascript">
							var f;        
					 		...
					 		var l;
					 		l = "'.$s_lname.'";
							alert("Details of Pharmacist \""+f+" "+l+"\" has been Updated Successfully!"); 
...
<html>
<head>
	<title> MANAGER UPDATE PHARMACIST PAGE </title>
	...
	<script type="text/javascript">
		...
		function on_load()
		{
			...
			update_pharmacist.address.value =add;
	...
	<div id="homepage">
		...
		<div id="mainarea">
				...
				<div id="mainbox" align="center">
					<h4> MANAGER - UPDATE PHARMACIST <br>
						<span>-- Only modify details that you want to update --</span>
					</h4>
				<form name="update_pharmacist" id="update_pharmacist" action="manager_update_pharmacist.php" method="post" autocomplete="OFF">
					...
					<table>
        				<tr> 
        					<th>Pharmacist ID</th> 
        					<td> <input type="text" name="id" value="<?php echo "$pid" ?>" placeholder="Pharmacist ID" oninvalid="validate(this);" oninput="validate(this);" required=""> </td>
        				</tr>
        				<tr>   
        					<th>Username </th> 
        				...
        				<tr>   
        					<th>Address </th> 
        					<td> <textarea name="address" rows="3" form="update_pharmacist" placeholder="Address" wrap="soft" oninvalid="validate(this);" oninput="validate(this);" required=""></textarea> </td> 
        				</tr>
        				<tr>   
        					<th>Mobile No. </th> 
					...
					<a href="manager_pharmacist.php" class="button">GO BACK</a>
